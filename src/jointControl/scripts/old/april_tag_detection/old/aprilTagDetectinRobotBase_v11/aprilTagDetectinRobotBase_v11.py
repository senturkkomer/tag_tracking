...
class Perception:
    ...
    def __init__(self):
        ...
        thread_main.start() 
    def get_axes_tag(self, corners):
        y_axis = (corners[0] + corners[1]) / 2
        x_axis = (corners[1] + corners[2]) / 2
        z_axis = np.cross(x_axis,y_axis)
        tag_axes = [x_axis, y_axis, z_axis]
        return tag_axes
        ...
        while True:
            ...
            try:
                if (self.center_x is not None) and (self.center_y is not None):
                    ...
                    self.corners_zed = np.zeros((4,3))
                    self.corners_zed[0][0], self.corners_zed[0][1], self.corners_zed[0][2], err1 = self.get_3d_position(self.corners[0][0], self.corners[0][1])
                    self.corners_zed[1][0], self.corners_zed[1][1], self.corners_zed[1][2], err2 = self.get_3d_position(self.corners[1][0], self.corners[1][1])
                    self.corners_zed[2][0], self.corners_zed[2][1], self.corners_zed[2][2], err3 = self.get_3d_position(self.corners[2][0], self.corners[2][1])
                    self.corners_zed[3][0], self.corners_zed[3][1], self.corners_zed[3][2], err4 = self.get_3d_position(self.corners[3][0], self.corners[3][1])
                    
                    self.center_zed = [center_x_zed, center_y_zed, center_z_zed]
                    self.tag_axes = self.get_axes_tag(self.corners_zed)
                    
                    R = trans.rotation_matrix_from_vectors(np.array((0,0,1)),self.tag_axes[2])
                    self.rx, self.ry, self.rz = trans.r_to_rpy(R)
    ...
    def get_3d_position(self, x, y):
        if self.xyz_array is not None :
            ...
            if 0 <= x < width and 0 <= y < height:
                center_x_zed, center_y_zed, center_z_zed = self.xyz_array[y, x]
                ...
                rospy.logerr(
                    f"Center coordinates out of bounds: center_x={x}, center_y={y}, width={width}, height={height}"
                )
    ...
    def broadcast_tag_frame(self,center_x_zed, center_y_zed, center_z_zed):
        try:  
            ...
            # R = trans.rotation_matrix_from_vectors(np.array((0, 0, 1)), self.tag_axes[2])
            # rx, ry, rz = trans.r_to_rpy(R)
            quaternion = tf.transformations.quaternion_from_euler(self.rx,self.ry, self.rz)  # (Roll, Pitch, Yaw) = (0, 90°, 0)
            ...
            t1.header.frame_id = "zedm_base_link"
            t1.child_frame_id = "april_tag_x_axis"
            t1.transform.translation.x = self.tag_axes[0][0]
            t1.transform.translation.y = -self.tag_axes[0][2]
            t1.transform.translation.z = self.tag_axes[0][1]
            ...
            t2.header.frame_id = "zedm_base_link"
            t2.child_frame_id = "april_tag_y_axis"
            t2.transform.translation.x = self.tag_axes[1][0]
            t2.transform.translation.y = -self.tag_axes[1][2]
            t2.transform.translation.z = self.tag_axes[1][1]
            ...
            z_axis_point = geometry_msgs.msg.PointStamped()
            z_axis_point.point.x = self.tag_axes[2][0]
            z_axis_point.point.y = self.tag_axes[2][1]
            z_axis_point.point.z = self.tag_axes[2][2]
            
            
            
            self.transform = self.tf_buffer.lookup_transform('base_link', 'april_tag', rospy.Time(0), rospy.Duration(2.0))
            self.transform2 = self.tf_buffer.lookup_transform('april_tag', 'zedm_base_link', rospy.Time(0), rospy.Duration(2.0))
            z_axis_in_tag = tf2_geometry_msgs.do_transform_point(z_axis_point, self.transform)
            t3 = geometry_msgs.msg.TransformStamped()
            t3.header.stamp = rospy.Time.now()
            t3.header.frame_id = "april_tag"
            t3.child_frame_id = "april_tag_z_axis"
            t3.transform.translation.x = z_axis_in_tag.point.x
            t3.transform.translation.y = -z_axis_in_tag.point.z
            t3.transform.translation.z = z_axis_in_tag.point.y
    ...
    def define_safety_zone(self):
        ...
        safety_zone_pose2.pose.position.z = -0.1
