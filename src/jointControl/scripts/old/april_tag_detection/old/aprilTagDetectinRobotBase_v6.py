...
class Perception:
    ...
    def thread_sub(self):
        ...
        t2 = threading.Thread(target=self.thread_sub2, name='sub2')
        t2.start() 
    ...
    def robot_at_target_pos(self):
        ...
        return not(math.sqrt(xdiff**2 + ydiff**2 + zdiff**2) > 0.05)
    def go_target_pos(self):
        if not self.robot_at_target_pos():
            ...
            print("robot is at target position")
...
if __name__ == "__main__":
    ...
    perception.t = threading.Thread(target=perception.thread_sub, name='sub')
    perception.t.start()
