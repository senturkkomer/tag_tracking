...
class RobotControl:
    ...
    def robot_go(self):
        while not rospy.is_shutdown():
            ...
            self.define_target_pose()
        if self.target_pos_in_base_link is not None:    
            can_transform = self.tf_buffer.can_transform(BASE_LINK, TARGET_POSE_FRAME, rospy.Time.now())
            if can_transform:
                ...
                rospy.logwarn(BASE_LINK, "could not be transformed to ", TARGET_POSE_FRAME)
        else:
